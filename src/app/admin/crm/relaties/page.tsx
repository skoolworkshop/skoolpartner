...
const FILTERS: { key: string; label: string; lifecycle?: Lifecycle | "alles"; stil?: number }[] = [
      ...
      >
        <span className="min-w-0 flex-1 basis-full sm:basis-auto">
          ...
          <span className="block truncate text-sm text-muted">
            {relatie.city ?? "plaats onbekend"} · {relatie.aantalBoekingen}{" "}
    ...
    return (
      <>
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <h1 className="text-[30px]">Relaties</h1>
          <MerkSchakelaar actief={merk} />
        </div>
        <Card>
          <EmptyState
            title="Suri Impact werkt niet met relaties"
            description="Het Breekjaar wordt per persoon verkocht, niet per organisatie. Ga naar Reisperiodes om de deelnemers te zien, of zet de schakelaar op Skool Workshop."
            action={
              <Link
                href="/admin/crm/suri"
  ...
  return (
    <>
      ...
      <p className="mb-6 max-w-2xl text-muted">
        Dit overzicht beantwoordt de vraag met wie je iets moet doen. Voor de volledige gegevens van
        een school, met boekingen, facturen, punten en tegoed, klik je door naar de organisatie.
      </p>
      ...
      <Card>
        <CardHeader
          ...
          action={
            <form className="flex flex-wrap items-center gap-2" action="/admin/crm/relaties">
        ...
        <div className="flex flex-wrap gap-1.5 border-b border-line-soft px-5 py-3">
          {FILTERS.map((f) => (
            <Link
              key={f.key}
              href={`/admin/crm/relaties?filter=${f.key}${zoek ? `&zoek=${encodeURIComponent(zoek)}` : ""}`}
