-- =============================================================================
-- SkoolPartner - schoon opnieuw beginnen met twee accounts
-- =============================================================================
-- LET OP: dit script gooit ALLE gegevens weg. Alle organisaties, boekingen,
-- facturen, punten, berichten, resultaten en ook alle accounts. Draai het
-- alleen op het testproject, nooit op een omgeving met echte klanten.
--
-- Wat je erna hebt:
--
--   contact@skoolworkshop.nl    beheerder, ziet alles van alle klanten,
--                               komt na inloggen in de beheeromgeving
--   planning@skoolworkshop.nl   klant bij De Goudse Waarden,
--                               komt na inloggen in het klantportaal
--
--   Drie organisaties, waarvan er twee bewust geen gebruiker hebben, zodat je
...
-- je een tabel met de puntensaldi ter controle. Opnieuw draaien mag.
-- =============================================================================


-- -----------------------------------------------------------------------------
-- Deel 1. Alles leegmaken
-- -----------------------------------------------------------------------------
do $$
begin
  -- Volgorde is belangrijk: eerst wat naar iets anders verwijst.
  delete from public.workshop_result_files;
  delete from public.workshop_results;
  ...
  delete from public.booking_sources;
  ...
  -- De bestanden van workshopresultaten. De rijen gaan hier weg; staat er nog
  -- iets in de opslag zelf, dan haal je dat weg via Storage in Supabase.
  if to_regclass('storage.objects') is not null then
    delete from storage.objects where bucket_id = 'workshop-resultaten';
  end if;

  -- En als laatste de accounts. Profielen hangen hieraan en verdwijnen mee.
  delete from auth.users;

  raise notice 'Alles leeggemaakt.';
end $$;


-- -----------------------------------------------------------------------------
-- Deel 2. De twee accounts aanmaken
-- -----------------------------------------------------------------------------
-- Lukt dit niet, dan is dat geen ramp. Supabase verandert af en toe iets in de
-- eigen accounttabellen. Je ziet dan een melding onderaan. Ga in dat geval naar
-- de inlogpagina van SkoolPartner, maak daar een account aan met beide
-- adressen, en draai daarna alleen deel 3 nog een keer.
-- -----------------------------------------------------------------------------
do $$
declare
  v_adressen text[] := array['contact@skoolworkshop.nl', 'planning@skoolworkshop.nl'];
  v_adres    text;
  v_id       uuid;
begin
  foreach v_adres in array v_adressen loop
    if exists (select 1 from auth.users where lower(email) = lower(v_adres)) then
      raise notice 'Account % bestond al.', v_adres;
      continue;
    end if;
    ...
    begin
      ...
      insert into auth.identities (
        ...
        v_id::text, v_id,
        jsonb_build_object('sub', v_id::text, 'email', lower(v_adres), 'email_verified', true),
        'email', now(), now(), now()
      );

      raise notice 'Account % aangemaakt. Inloggen gaat met een inloglink, er is geen wachtwoord.', v_adres;
    exception when others then
      raise notice 'Kon % niet automatisch aanmaken (%). Maak dit account aan via de inlogpagina en draai daarna deel 3 opnieuw.', v_adres, sqlerrm;
    end;
  end loop;
end $$;
...
-- -----------------------------------------------------------------------------
-- Deel 3. Rollen vastleggen en testdata neerzetten
-- -----------------------------------------------------------------------------
do $$
declare
  -- >>> PAS DIT AAN <<<
  ...
  v_email    text := 'planning@skoolworkshop.nl';   -- de klant
  v_beheer   text := 'contact@skoolworkshop.nl';    -- de beheerder
  v_beheerder uuid;
...
begin
  ---------------------------------------------------------------------------
  -- 1. De twee accounts opzoeken en hun rol vastleggen
  ---------------------------------------------------------------------------
  select id into v_user      from auth.users where lower(email) = lower(v_email);
  select id into v_beheerder from auth.users where lower(email) = lower(v_beheer);

  if v_user is null or v_beheerder is null then
    select string_agg(email, ', ' order by created_at) into v_bekend from auth.users;
    raise exception
      'Nog niet beide accounts gevonden. Gezocht naar % en %. Bekend: %. Log met het ontbrekende adres een keer in via de inlogpagina en draai daarna alleen dit derde deel opnieuw.',
      v_beheer, v_email, coalesce(v_bekend, 'nog geen enkel account');
  end if;
  ...
  -- De klant hoort bij De Goudse Waarden. De andere twee organisaties hebben
  -- bewust geen gebruiker: zo zie je in de beheeromgeving ook klanten die zelf
  -- nog niet inloggen.
  insert into public.organization_members
    (organization_id, user_id, role, status, source, approved_by, approved_at)
  values
    (v_org1, v_user, 'beheerder', 'active', 'admin_manual', v_beheerder, now())
  on conflict (organization_id, user_id) do update
    set status = 'active', role = 'beheerder', approved_at = now();

  -- De beheerder is nergens lid. Die werkt via /admin.
  delete from public.organization_members where user_id = v_beheerder;
  ...
  if exists (
    ...
    values (v_org1, v_email, 'Sanne de Vries', v_user, true, now(), v_beheerder);
  end if;
  ...
  raise notice 'Klaar. % is beheerder, % is klant bij De Goudse Waarden.', v_beheer, v_email;
end $$;
